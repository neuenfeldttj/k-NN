...
public class ProfileKnnByteVectorQuery extends KnnByteVectorQuery {
    ...
    protected TopDocs approximateSearch(
        ...
        if (profilers != null) {
            Timer timer = profilers.getCurrentQueryProfiler().getProfileBreakdown(this).context(context).getTimer(KNNQueryTimingType.ANN_SEARCH);
            timer.start();
            try {
    ...
    protected TopDocs exactSearch(LeafReaderContext context, DocIdSetIterator acceptIterator, QueryTimeout queryTimeout)
        throws IOException {
        if (profilers != null) {
            Timer timer = profilers.getCurrentQueryProfiler().getProfileBreakdown(this).context(context).getTimer(KNNQueryTimingType.EXACT_SEARCH);
            timer.start();
            try {
